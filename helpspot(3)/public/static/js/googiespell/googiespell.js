...
GoogieSpell.prototype.updateOrginalText = function(offset, old_value, new_value, id) {
    ...
    this.orginal_text = part_1 + new_value + part_2;
 	//HELPSPOT CHANGE
    if(navigator.appVersion.match(/\bKHTML\b/)){
    	this.text_area.innerHTML = this.orginal_text;
    }else{
...
GoogieSpell.prototype.flashNoSpellingErrorState = function(on_finish) {
    ...
    if(this.main_controller) {
        AJS.hideElement(this.switch_lan_pic);
		 ...
		 //HelpSpot Change, image to 1 px
        var dummy = AJS.IMG({'src': this.img_dir + "blank.gif", 'style': 'height: 1px; width: 1px;'});
        var rsm = AJS.SPAN();
...
GoogieSpell.prototype.appendIndicator = function(elm) {
	...
	if(this.show_indicator){
        ...
        var rsm = AJS.SPAN();
        rsm.innerHTML = this.lang_checking;
        rsm.className = "googie_checking";
        this.indicator = rsm;
         AJS.RCN(this.spell_span, rsm);
        //AJS.insertBefore(rsm, elm);
    }
}
