...
class OLE extends PEAR
{
     ...
     *
     * @param int $pps_wk_start   Position inside the OLE file where PPS WK's start
     * @param int $big_block_size Size of big blobks in the OLE file
     * @return mixed true on success, PEAR_Error on failure
     */
    public function _readPpsWks($pps_wk_start, $big_block_size)
    {
        ...
        while (1) {
            ...
            $this->_list[] = new OLE_PPS(null, '', $type[''], $prev[''], $next[''],
                                         $dir[''], self::OLE2LocalDate($time_1st),
                                         self::OLE2LocalDate($time_2nd),
