...
class XML_Util
{
     ...
     * @param    int setting for entities in attribute values (one of XML_UTIL_ENTITIES_XML, XML_UTIL_ENTITIES_XML_REQUIRED, XML_UTIL_ENTITIES_HTML)
    ...
    public static function getXMLDeclaration($version = '1.0', $encoding = null, $standalone = null)
    {
        ...
        return sprintf('<?xml%s?>', self::attributesToString($attributes, false));
    }
     ...
     * @param    bool|array $sort              sort attribute list alphabetically, may also be an assoc array containing the keys 'sort', 'multiline', 'indent', 'linebreak' and 'entities'
     * @param    bool       $multiline         use linebreaks, if more than one attribute is given
     ...
     * @param    int       $entities          setting for entities in attribute values (one of XML_UTIL_ENTITIES_NONE, XML_UTIL_ENTITIES_XML, XML_UTIL_ENTITIES_XML_REQUIRED, XML_UTIL_ENTITIES_HTML)
            ...
            if (! $multiline || count($attributes) == 1) {
                foreach ($attributes as $key => $value) {
                    if ($entities != XML_UTIL_ENTITIES_NONE) {
                        ...
                        $value = self::replaceEntities($value, $entities);
                    }
                    $string .= ' '.$key.'="'.$value.'"';
                }
            } else {
     ...
     * @param    int $mode Whether to collapse all empty tags (XML_UTIL_COLLAPSE_ALL) or only XHTML (XML_UTIL_COLLAPSE_XHTML_ONLY) ones.
     ...
     * @param    int $replaceEntities   whether to replace XML special chars in content, embedd it in a CData section or none of both
     * @param    bool $multiline         whether to create a multiline tag where each attribute gets written to a single line
     ...
     */
    public static function createTag($qname, $attributes = [], $content = null, $namespaceUri = null, $replaceEntities = XML_UTIL_REPLACE_ENTITIES, $multiline = false, $indent = '_auto', $linebreak = "\n")
    {
        $tag = [
                     'qname'      => $qname,
        ...
        return self::createTagFromArray($tag, $replaceEntities, $multiline, $indent, $linebreak);
    }
    ...
    public static function createTagFromArray($tag, $replaceEntities = XML_UTIL_REPLACE_ENTITIES, $multiline = false, $indent = '_auto', $linebreak = "\n")
    {
        if (isset($tag['content']) && ! is_scalar($tag['content'])) {
            return self::raiseError('Supplied non-scalar value as tag content', XML_UTIL_ERROR_NON_SCALAR_CONTENT);
        }

        if (! isset($tag['qname']) && ! isset($tag['localPart'])) {
            return self::raiseError('You must either supply a qualified name (qname) or local tag name (localPart).', XML_UTIL_ERROR_NO_TAG_NAME);
        }
        ...
        if (! isset($tag['qname'])) {
            ...
            $parts = self::splitQualifiedName($tag['qname']);
        ...
        $attList = self::attributesToString($tag['attributes'], true, $multiline, $indent, $linebreak, $replaceEntities);
        if (! isset($tag['content']) || (string) $tag['content'] == '') {
            ...
            switch ($replaceEntities) {
                ...
                case XML_UTIL_CDATA_SECTION:
                    $tag['content'] = self::createCDataSection($tag['content']);

                    break;
                default:
                    $tag['content'] = self::replaceEntities($tag['content'], $replaceEntities);
     ...
     */
    public static function createStartElement($qname, $attributes = [], $namespaceUri = null, $multiline = false, $indent = '_auto', $linebreak = "\n")
    {
        // if no attributes hav been set, use empty attributes
        ...
        if ($namespaceUri != null) {
            $parts = self::splitQualifiedName($qname);
        }
        ...
        // create attribute list
        $attList = self::attributesToString($attributes, true, $multiline, $indent, $linebreak);
    ...
    public static function isValidName($string)
    {
        ...
        if (! preg_match("/^[[:alnum:]_\-.]$/", $string[0])) {
            return self::raiseError('XML names may only start with letter or underscore', XML_UTIL_ERROR_INVALID_START);
        }

        // check for invalid chars
        if (! preg_match("/^([a-zA-Z_]([a-zA-Z0-9_\-\.]*)?:)?[a-zA-Z_]([a-zA-Z0-9_\-\.]+)?$/", $string)) {
            return self::raiseError('XML names may only contain alphanumeric chars, period, hyphen, colon and underscores', XML_UTIL_ERROR_INVALID_CHARS);
        }
        // XML name is valid
        return true;
    }
