...
class Net_UserAgent_Detect
{
    ...
    public function &singleton($in_userAgent = null, $in_detect = null)
    {
        ...
        if (! isset($instance)) {
            $instance = new self($in_userAgent, $in_detect);
        }
    ...
    {
        static $hasRun;
        $options = &self::_getStaticProperty('options');
        ...
        $quirks = &self::_getStaticProperty('quirks');
        $quirks = [
        ...
        // in the script.
        $features = &self::_getStaticProperty('features');
        $features = [
                'javascript'   => false,
                'dhtml'        => false,
                'dom'          => false,
        ...
        $detectOptions = [NET_USERAGENT_DETECT_BROWSER,
        ...
        if ($detectFlags[NET_USERAGENT_DETECT_ALL] ||
            ...
            elseif (($browser['ns4'] && ($version > 4.05)) || $browser['ie5up'] || $browser['hotjava3up']) {
                ...
                self::setFeature('javascript', 1.5);
            }
        }
        ...
        if ($detectFlags[NET_USERAGENT_DETECT_ALL] ||
            ...
            if ($browser['konq']) {
                self::setQuirk('empty_file_input_value');
            }

            if ($browser['ie']) {
                self::setQuirk('cache_ssl_downloads');
            }

            if ($browser['ie6']) {
                self::setQuirk('scrollbar_in_way');
            }

            if ($browser['ie5']) {
                self::setQuirk('break_disposition_header');
            }

            if ($browser['ie7']) {
                self::setQuirk('popups_disabled');
            }

            if ($browser['ns6']) {
                self::setQuirk('popups_disabled');
                self::setQuirk('must_cache_forms');
            }

            if ($browser['nav'] && $subVersion < .79) {
                self::setQuirk('nested_table_render_bug');
            }
        }
        ...
        if ($detectFlags[NET_USERAGENT_DETECT_ALL] || $detectFlags[NET_USERAGENT_DETECT_ACCEPT]) {
            ...
            self::setAcceptType((array) $mimetypes, 'mimetype');
     ...
     *
     * @return bool whether or not the browser satisfies this flag
     */
     ...
     * @param  string $in_match flag to lookup
     *
     * @return bool whether or not the OS satisfies this flag
     */
    ...
    {
        return (bool) self::getQuirk($in_quirk);
    }
    ...
    {
        return (bool) self::getFeature($in_feature);
    }
    ...
    public function getAcceptType($in_expectList, $in_type)
    {
        ...
        if ($type == 'mimetype' || $type == 'language' || $type == 'charset' || $type == 'encoding') {
            $typeArray = &self::_getStaticProperty($type);
    ...
    {
        return (bool) self::getAcceptType((array) $in_value, $in_type);
    }
