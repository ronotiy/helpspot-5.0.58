...
class Auth_Container_LDAP extends Auth_Container
{
     ...
     */
    public function Auth_Container_LDAP($params)
    {
     ...
     * @return array
     */
    public function _setV12OptionsToV13($array)
    {
     ...
     */
    public function _scope2function($scope)
    {
        switch ($scope) {
        case 'one':
            $function = 'ldap_list';
     ...
     * @return bool
     */
    public function checkGroup($user)
    {
        ...
        $func_params = [$this->conn_id, $search_basedn, $filter,
                             [$this->options['memberattr']], ];
     ...
     */
    public function _quoteFilterString($filter_str)
    {
        $metas = ['\\',  '*',  '(',  ')',   "\x00"];
        $quoted_metas = ['\\\\', '\*', '\(', '\)', "\\\x00"];
