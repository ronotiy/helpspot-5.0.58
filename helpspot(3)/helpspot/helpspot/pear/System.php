...
class System
{
     ...
     */
    public function _dirToStruct($sPath, $maxinst, $aktinst = 0)
    {
        $struct = ['dirs' => [], 'files' => []];
        ...
        if ($aktinst < $maxinst || $maxinst == 0) {
            foreach ($list as $val) {
                ...
                if (is_dir($path)) {
                    $tmp = self::_dirToStruct($path, $maxinst, $aktinst + 1);
    ...
    {
        $opts = self::_parseArgs($args, 'rf'); // "f" do nothing but like it :-)
    ...
    public function which($program, $fallback = false)
    {
        ...
        $exe_suffixes = OS_WINDOWS ? ['.exe', '.bat', '.cmd', '.com'] : [''];
    ...
    public function find($args)
    {
        ...
        $path = self::_dirToStruct($dir, $depth);
        if ($do_files && $do_dirs) {
