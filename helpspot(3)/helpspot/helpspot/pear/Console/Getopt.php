    ...
    public function doGetopt($version, $args, $short_options, $long_options = null)
    {
        ...
        while (list($i, $arg) = each($args)) {
            ...
            if ($arg[0] != '-' || (strlen($arg) > 1 && $arg[1] == '-' && ! $long_options)) {
                ...
                $error = self::_parseLongOption(substr($arg, 2), $long_options, $opts, $args);
                if (PEAR::isError($error)) {
                    return $error;
                }
            } else {
                $error = self::_parseShortOption(substr($arg, 1), $short_options, $opts, $args);
                if (PEAR::isError($error)) {
                    return $error;
                }
            }
            ...
            if (strlen($spec) > 1 && $spec[1] == ':') {
                if (strlen($spec) > 2 && $spec[2] == ':') {
                    ...
                    if ($i + 1 < strlen($arg)) {
                        ...
                        /* Else use the next argument. */; else {
