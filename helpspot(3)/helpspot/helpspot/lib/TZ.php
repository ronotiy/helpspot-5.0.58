...
$tz_country = [
  ...
  ['iso'=>'AQ', 'name'=>'ANTARCTICA', 'printable_name'=>'Antarctica', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'BV', 'name'=>'BOUVET ISLAND', 'printable_name'=>'Bouvet Island', 'iso3'=>null, 'numcode'=>null],
  ['iso'=>'BR', 'name'=>'BRAZIL', 'printable_name'=>'Brazil', 'iso3'=>'BRA', 'numcode'=>76],
  ['iso'=>'IO', 'name'=>'BRITISH INDIAN OCEAN TERRITORY', 'printable_name'=>'British Indian Ocean Territory', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'CX', 'name'=>'CHRISTMAS ISLAND', 'printable_name'=>'Christmas Island', 'iso3'=>null, 'numcode'=>null],
  ['iso'=>'CC', 'name'=>'COCOS (KEELING) ISLANDS', 'printable_name'=>'Cocos (Keeling) Islands', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'TF', 'name'=>'FRENCH SOUTHERN TERRITORIES', 'printable_name'=>'French Southern Territories', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'HM', 'name'=>'HEARD ISLAND AND MCDONALD ISLANDS', 'printable_name'=>'Heard Island and Mcdonald Islands', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'YT', 'name'=>'MAYOTTE', 'printable_name'=>'Mayotte', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'PS', 'name'=>'PALESTINIAN TERRITORY, OCCUPIED', 'printable_name'=>'Palestinian Territory, Occupied', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'CS', 'name'=>'SERBIA AND MONTENEGRO', 'printable_name'=>'Serbia and Montenegro', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'GS', 'name'=>'SOUTH GEORGIA AND THE SOUTH SANDWICH ISLANDS', 'printable_name'=>'South Georgia and the South Sandwich Islands', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'TL', 'name'=>'TIMOR-LESTE', 'printable_name'=>'Timor-Leste', 'iso3'=>null, 'numcode'=>null],
  ...
  ['iso'=>'UM', 'name'=>'UNITED STATES MINOR OUTLYING ISLANDS', 'printable_name'=>'United States Minor Outlying Islands', 'iso3'=>null, 'numcode'=>null],
...
//Define TZ for installer
if (! defined('cHD_TIMEZONE_OVERRIDE')) {
    define('cHD_TIMEZONE_OVERRIDE', 'America/New_York');
}

$timezoneSelect = '<select name="cHD_TIMEZONE_OVERRIDE" id="cHD_TIMEZONE_OVERRIDE">';
foreach ($tz_country as $k=>$info) {
    $tzlist = DateTimeZone::listIdentifiers(DateTimeZone::PER_COUNTRY, $info['iso']);
    $timezoneSelect .= '<optgroup label="'.$info['printable_name'].'">';
    foreach ($tzlist as $k=>$tz) {
        $temptz = new DateTimeZone($tz);
        $tznow = $date = new DateTime('now', $temptz);
        $tzinfo = $temptz->getLocation();

        //Of course the US timezones are a complete mess we need to manually filter some of the junk ones
        if (! in_array($tz, ['America/Detroit', 'America/Indiana/Marengo', 'America/Indiana/Petersburg', 'America/Indiana/Tell_City', 'America/Indiana/Vevay', 'America/Indiana/Vincennes', 'America/Indiana/Winamac', 'America/Shiprock', 'America/Boise', 'America/Juneau', 'America/Kentucky/Monticello', 'America/Menominee', 'America/Nome', 'America/North_Dakota/New_Salem', 'America/Yakutat', 'America/Indiana/Indianapolis', 'America/Indiana/Knox', 'America/Kentucky/Louisville', 'America/North_Dakota/Center'])) {
            $tzname = explode('/', $tz);
            array_shift($tzname);
            $tzname = implode('/', $tzname);
            $tzname = str_replace('_', ' ', $tzname);

            $tztrans = $temptz->getTransitions(time(), time());
            //$tzname = $tzname . ' ('.$tztrans[0]['abbr'].')';

            $tzname = str_replace(' ', '&nbsp;', str_pad($tzname, 22, ' ')).' | '.str_replace(' ', '&nbsp;', str_pad($tztrans[0]['abbr'], 4)).' | '.$tznow->format('h:i a (H:i)');

            $selected = ($tz == hs_setting('cHD_TIMEZONE_OVERRIDE')) ? 'selected' : '';
            $timezoneSelect .= '<option style="font-family:Courier;" value="'.$tz.'" '.$selected.'>'.$tzname.'</option>';
            //$tzscript .= 'TIMEZONE_ARRAY["'.$tzinfo['country_code'].'###'.$tz.'"] = new Array("'.$tz.'","'.$tznow->format('m/d').'","'.$tznow->format("g:i a (H:i)").'","'.$tznow->format('P').'","'.$tzname.'");'."\n";
        }
    }
    $timezoneSelect .= '</optgroup>';
}
